...
if(isset($_REQUEST['submit'])){
		...
		if( $father_name=="" || $FatherMobile=="" || $mother_name=="" || $LocalGuardianName=="" || $LocalGuardianContact=="" || $RelationwithLocalGuardian==""){
			header("Location:../guardian.php?error=Field Must Not Be Empty!!!&email=$email");
		}else{
		...
		if($run== true){
			header("Location:../guardian.php?msg=Data Updated Successfully.&email=$email");
		}else{
			header("Location:../guardian.php?error=Something Went Wrong.&email=$email");
		}
